...
'''Python 2.7.2 '''
import csv
import fundfees  # this a dictionary called fund_fees with the key
# being the health fund ID and the data being a tuple of the consult fee
# and the unit fee for each fund.
import test_string_mod  # this file contains a string variable
# called 'test_string_acc' which is the base html code for the account

pe_code = 20740
col_code = 20810
age_code = 25015
sick_code = 25000
tabl_html = '''<tr><td>%s</td><td>$</td><td style="text-align:right"> %.2f</td>
                    </tr>\n'''
# this is the repetetive html that goes into the Fees Table
count = 0
input_day = raw_input("input date in format --   ")
input_month = raw_input("input date in format --   ")
...
with open('/Users/jtair/Downloads/JT Patients 2016.csv', 'rb') as csvfile:
    ...
    for row in patlist:
        ...
        if ep_day == input_day and ep_month == input_month\
            ...
            html_output = test_string_mod.test_string_acc % \
                (patient, fund, ep_date, doctor, consult)
            ...
            ep_file.close()
print 'Number of accounts printed is {}'.format(count)  # final print out of number of accounts done
